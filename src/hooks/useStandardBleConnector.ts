...
localStorage.setItem('debug', 'llsync:*');

const blueToothAdapter = sdk.blueToothAdapter;
const ERROR_MESSAGES = sdk.ERROR_MESSAGES;
sdk.blueToothAdapter.addAdapter(StandardDeviceAdapter, true);
// sdk.blueToothAdapter._deviceAdapterFactoryMap[StandardDeviceAdapter.serviceId16] = StandardDeviceAdapter;
...
export const getStandardBleConnectStatusInfo = (
...
export const useStandardBleConnector = ({ deviceId, familyId = '' }) => {
  const { deviceInfo } = sdk;

  const deviceAdapterRef = useRef(null);
  const [connectStatusInfo, setConnectStatusInfo] = useState(
    getStandardBleConnectStatusInfo()
  );
  ...
  const getDeviceAdapter = () => {
    ...
    if (!deviceAdapter) {
      deviceAdapter = sdk.blueToothAdapter.getDeviceAdapter({
  ...
  const isDeviceReady = (deviceAdapter?: any) => {
  ...
  const connectDevice = async () => {
    try {
      ...
      if (!isDeviceReady(deviceAdapter)) {
          ...
          try {
            ...
            if (device) {
              try {
                ...
                console.warn('连接设备出错', err.message, err.stack);
                throw err;
              }
            }
          } catch (err) {
            console.warn('搜索设备出错', err);
          }
        }
        ...
        console.log('%c --connect device', 'color: yellow', deviceAdapter);
        if (!deviceAdapter.isConnected) {
          await deviceAdapter.connectDevice();
        }
        console.log('%c auth device', 'color: yellow', deviceAdapter);
  ...
  const deleteDevice = async () => {
    if (!isDeviceReady()) {
      const isConfirm = confirm(
        '确定要删除这个设备吗？当前设备未连接,删除后如需重新绑定需要初始化蓝牙设备。'
      );
