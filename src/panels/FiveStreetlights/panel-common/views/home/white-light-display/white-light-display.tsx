...
export function WhiteLightDisplay() {
  ...
  const moveDisplayDot = (clientX, clientY, endTouch) => {
    const wrap = document.getElementById('white-light-content');
    const point = document.getElementById('white-light-display-dot');
    const wrap_y = document.getElementById('white-light-content').getBoundingClientRect().y;
    ...
    const clr = calColor(theta * 180 / Math.PI);
  ...
  const calColor = (theta) => {
    ...
    // console.log(x);
    // console.log(y);
    const r = (1 - p) * x[0] + p * y[0];
    const g = (1 - p) * x[1] + p * y[1];
    const b = (1 - p) * x[2] + p * y[2];
