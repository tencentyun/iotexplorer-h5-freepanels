...
export function Position(props: LightColorProps) {
  ...
  const updatePosInfo = () => {
    ...
    let n = parseInt(dataUser / 1000.0 * 275);
    //0~50映射为130~180,50~275映射为-180~45
    let angle = n > 50 ? (n - 50 - 180) : n + 130;
    angle *= Math.PI / 180; // 角度转弧度
    let destX = centerX + Math.cos(angle) * wrap?.clientWidth / 2 * 0.77;
    let destY = centerY + Math.sin(angle) * wrap?.clientHeight / 2 * 0.77;
  ...
  const handleSelectColor = (e: React.MouseEvent) => {
    ...
    let dist = Math.sqrt((centerX - x) * (centerX - x) + (centerY - y) * (centerY - y));
    ...
    let val = (angle <= 45 ? angle + 180 + 50 : angle - 130);
    console.log(val);
