...
export function Home(props) {
  ...
  let [colorMin, colorMax] = [1, 100]; // 亮度
  let [tempMin, tempMax] = [2700, 6500]; // 色温

  const calcValue = (position, val, [min, max]) => {
    let value = Math.round(val + position.OffsetYPercent);
    let result = value >= max ? max : (value <= min ? min : value)
    return result;
  };
  ...
  const onTempMove = (position) => {
    let val = stepValue(calcValue(position, color_temp, [tempMin, tempMax]))
    setModel(val);
    setField({ ...field, attr: 'color_temp', color_temp: val })
  };

  const onTempEnd = (position) => {
    let value = stepValue(calcValue(position, color_temp, [tempMin, tempMax]));
    setModel(value);
  ...
  const onColorModeMove = (position) => {
    let val = calcValue(position, brightness, [colorMin, colorMax])
    setField({ ...field, attr: 'brightness', brightness: val })
  };

  // 存储值
  const onColorModeEnd = (position) => {
    let value = calcValue(position, brightness, [colorMin, colorMax]);
    ...
    <div className='content-bottom'>
      <div className={classNames("change-panel")}>
        ...
        <div className={classNames('touch-panel', `mode-${color_mode}`)} style={{ zIndex: 2, background: 'transparent', position: 'absolute', top: 0 }}>
          <Move className="touch-item" key="color_temp" onMove={onTempMove} onEnd={onTempEnd} beginValue={tempMin} endValue={tempMax} >
            <>
              <Icon name={`gesture-ver-${parseInt(color_mode) === 1 || parseInt(color_mode) === 3 ? 'blank' : 'blank'}`} />
          ...
          <Move className="touch-item" key="brightness" onMove={onColorModeMove} onEnd={onColorModeEnd} beginValue={colorMin} endValue={colorMax}>
            <>
              <Icon name={`gesture-ver-${parseInt(color_mode) === 1 || parseInt(color_mode) === 3 ? 'blank' : 'blank'}`} />
        ...
        <div className="mode-list">
          {getOptions(templateMap, 'color_mode').map(({ label, value }, index) => (
            <div
              ...
              className="mode-item"
              onClick={() => {
                deviceData?.power_switch && doControlDeviceData('color_mode', parseInt(value))
              }}>
