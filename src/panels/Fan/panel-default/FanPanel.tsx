...
export function FanPanel({
  ...
  const [sliderValue, setSliderValue] = useState((deviceData.windspeed * 100)
  ...
  const labelLength =    100 / (Object.keys(templateMap.windspeed.define.mapping).length - 1);
  // 根据拖动值获取风的等级
  const getLabel = (v) => {
    // 根据map的分段来划分档位
    const label = Math.max(Math.round(v / labelLength), 0);
    return label;
  };
  // 根据风的等级获取拖动值
  const getSpeed = (label) => {
    const v = label * labelLength;
    return v;
  };
  const changeSpeed = (v) => {
    setSliderValue(v);
    const label = getLabel(v);
    const newValue = getSpeed(label);
  ...
  const formatTip = (v) => {
    const windSpeedMap = templateMap.windspeed.define.mapping;
    const label = getLabel(v);
            ...
            <div className="timer-btn">
              {
                ...
                Object.keys(templateMap.timer.define.mapping).map((item: string | number) => (
                      <div
