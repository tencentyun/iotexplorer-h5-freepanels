  ...
  return (
    <article className="p-side">
      ...
      <div className="head-switch" onClick={() => {
        // 暂停再次点击则开启 开启状态则暂停
        props.doControlDeviceData({ control: props?.deviceData?.control === 'open' ? 'pause' : 'open' });
        sdk.deviceData.control = props?.deviceData?.control === 'open' ? 'pause' : 'open';
        control === 'open' ? ref.current.pause() : ref.current.open();
      }}>
        <Icon className="switch-icon" name={props?.deviceData?.control === 'open' ? 'stop' : 'begin'} size="large" />
      </div>
